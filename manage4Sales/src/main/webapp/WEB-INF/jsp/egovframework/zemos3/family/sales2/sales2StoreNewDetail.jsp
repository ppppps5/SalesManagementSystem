	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			<c:forEach items="${item}" var="item" varStatus="status">
			 itemCnt.push("useYn"+"${status.count}");
			</c:forEach>
			 
			<c:forEach items="${selectStoreNewDetailItemList}" var="item" varStatus="status">
			 for(j = 0; j<itemCnt.length; j++){
				if("${item.itemSeq}"==$("#"+itemCnt[j]).val()){
					if("${item.useYn}"=="Y"){
						$("#"+itemCnt[j]).prop("checked", true);
					}
				}
			 }
			</c:forEach>
			
			<c:forEach items="${selectStoreNewSmList}" var="item" varStatus="status">
				smCnt.push({"sabun" : "${item.sabun}", "uploadYn" : "${item.uploadYn}", "approvalYn" : "${item.approvalYn}", "deleteYn" : "${item.deleteYn}"});
			</c:forEach>
			<c:forEach items="${userList}" var="item" varStatus="status">
				for(k = 0; k<smCnt.length; k++){
					if("${item.USER_SABUN}" == smCnt[k].sabun && smCnt[k].deleteYn == "N"){
						var sabun = "${item.USER_SABUN}";
						var userName = "${item.USER_NAME}";
						if(smCnt[k].uploadYn == "Y"){
							var uploadYn = "checked='checked'";
						}else{
							var uploadYn = "";
						}
						if(smCnt[k].approvalYn == "Y"){
							var approvalYn = "checked='checked'";
						}else{
							var approvalYn = ""; 
						}
						var html = "";
						...
						html += "<td style = 'padding:0;'><input type='checkbox' id = '"+sabun+"uploadYn'"+uploadYn+"><label id='uploadYnChkLabel' for='"+sabun+"uploadYn'>&nbsp;</label></td>"
						html += "<td style = 'padding:0;'><input type='checkbox' id = '"+sabun+"approvalYn'"+approvalYn+"><label id='approvalYnChkLabel' for='"+sabun+"approvalYn'>&nbsp;</label></td>"
			...
			var unitSeq1 = "${selectStoreDetail[0].unitSeq1}";
			var unitSeq2 = "${selectStoreDetail[0].unitSeq2}";
			var unitSeq3 = "${selectStoreDetail[0].unitSeq3}";
			var unitSeq4 = "${selectStoreDetail[0].unitSeq4}";
			var unitSeq5 = "${selectStoreDetail[0].unitSeq5}";
			var unit1Yn = "${selectStoreDetail[0].unit1Yn}";
			var unit2Yn = "${selectStoreDetail[0].unit2Yn}";
			var unit3Yn = "${selectStoreDetail[0].unit3Yn}";
			var unit4Yn = "${selectStoreDetail[0].unit4Yn}";
			var unit5Yn = "${selectStoreDetail[0].unit5Yn}";
			
			<c:forEach items="${unit}" var="item" varStatus="status">
    		if("${item.useYn}"=='Y'){
    			if("${item.unitSeq}" == unitSeq1){
    				if(unit1Yn == "Y"){
    					$("#<c:out value="${item.unitNm}"/>").prop("checked", true);
    				}
    			}else if("${item.unitSeq}" == unitSeq2){
    				if(unit2Yn == "Y"){
    					$("#<c:out value="${item.unitNm}"/>").prop("checked", true);
    				}
    			}else if("${item.unitSeq}" == unitSeq3){
    				if(unit3Yn == "Y"){
    					$("#<c:out value="${item.unitNm}"/>").prop("checked", true);
    				}
    			}else if("${item.unitSeq}" == unitSeq4){
    				if(unit4Yn == "Y"){
    					$("#<c:out value="${item.unitNm}"/>").prop("checked", true);
    				}
    			}else if("${item.unitSeq}" == unitSeq5){
    				if(unit5Yn == "Y"){
    					$("#<c:out value="${item.unitNm}"/>").prop("checked", true);
    				}
    			}
			...
			$("#storeName").on("focusout", function() {
				...
				if("${selectStore[0].storeNm}" != "${item.storeNm}"){
		...
		function fnStoreUpdate() {
			...
			params.storeDetailSeq = "${selectStoreDetail[0].storeDetailSeq}";
			fn_loading_show();
			
			$.ajax({
				url 		: gvContextPath + "/zemos3/family/sales2/sales2StoreNewUpdate.do",
				type		: 'POST',
			    dataType 	: 'json',
			    data		: params
		...
		function fnAddSM(){
			...
			html += "<td style = 'padding:0;'><a href = '#' onclick = 'javascript:fnDelete("+sabun+")'><img src = '/zenielMWeb/images/egovframework/zemos3/delete.png'></a>"
			html += "<input type = 'hidden' value ='"+sabun+"'></td></tr>"
			$("#sm").html(html);
			$("#userListDiv").hide();
			$("#userList").val("");
...
<body>
    ...
    <div class="zemos_title1">
		<span class="zemos_title1_middle" onclick="javascript:fn_location_href();return false;"> ${storeUseName[0].storeUseNm}수정</span>
	      ...
	      <div>
			        ...
			        <tbody>
			        	<tr> 
			        		...
			        		<td style="text-align: left;">
			        			<input id="storeName" name="storeName" type="text" class="zemos_input1" style="float: left; width:100%;" placeholder='<c:out value="${storeUseName[0].storeUseNm}"></c:out>명 입력' maxlength="100" value="<c:out value="${selectStore[0].storeNm}"/>">
			        	...
			        	<tr>
			        		...
			        		<td style="text-align: left;">
		        				<input type="checkbox" id="offYn" name="offYn" value="offYn" <c:if test="${selectStore[0].offYn == 'Y'}">checked="checked"</c:if>>
		        				<label id="offYnChkLabel" for="offYn">오프라인</label>
		        				&nbsp;
		        				<input type="checkbox" id="onYn" name="onYn" value="onYn" <c:if test="${selectStore[0].onYn == 'Y'}">checked="checked"</c:if>>
			        	...
			        	<tr>
			        		...
			        		<td style="text-align: left;">
			        			<select id="taxYn" name="taxYn" class="zemos_select1" style="width:100%;">
			        				<option value="Y" <c:if test="${selectStore[0].taxYn == 'Y'}">selected="selected"</c:if>>과세</option>
									<option value="N" <c:if test="${selectStore[0].taxYn == 'N'}">selected="selected"</c:if>>면세</option>
								</select>
			        		</td>
			        	</tr>
			        	<tr>
			        	...
			        	<tr>
			        		...
			        		<td style="text-align: left;">
		        				<input type="checkbox" id="classYn" name="classYn" <c:if test="${selectStore[0].classyn == 'Y'}">checked="checked"</c:if>>
				        	...
				        	<tr class="classAll" style="display: none;">
				        		...
				        		<td style="text-align: left;">
				        			<select id="class1" name="class1" class="zemos_select1" style="width:100%;" disabled="disabled">
				        				...
				        				<c:if test="${item.useYn == 'Y'}">
											<option value="${item.class1Seq}" <c:if test="${selectStore[0].class1 == item.class1Seq}">selected="selected"</c:if>>${item.class1Nm}</option>
				        				</c:if>
										</c:forEach>
									</select>
				        		</td>
				        	...
				        	<tr class="classAll" style="display: none;">
				        		...
				        		<td style="text-align: left;">
				        			<select id="class2" name="class2" class="zemos_select1" style="width:100%;" disabled="disabled">
										...
										<c:if test="${item.useYn == 'Y'}">
											<option value="${item.class2Seq}" <c:if test="${selectStore[0].class2 == item.class2Seq}">selected="selected"</c:if>>${item.class2Nm}</option>
										</c:if>
										</c:forEach>
									</select>
				        		</td> 
				        	...
				        	<tr class="classAll" style="display: none;">
				        		...
				        		<td style="text-align: left;">
				        			<select id="class3" name="class3" class="zemos_select1" style="width:100%;" disabled="disabled">
										...
										<c:if test="${item.useYn == 'Y'}">
											<option value="${item.class3Seq}" <c:if test="${selectStore[0].class3 == item.class3Seq}">selected="selected"</c:if>>${item.class3Nm}</option>
										</c:if>
										</c:forEach>
									</select>
				        		</td> 
				        		...
				        		<td style="text-align: left; font-size: 12px;">${class4Nm[0].class4UseNm}</td>
					       		<td style="text-align: left;">
				        			<select id="class4" name="class4" class="zemos_select1" style="width:100%;" disabled="disabled">
										...
										<c:if test="${item.useYn == 'Y'}">
											<option value="${item.class3Seq}" <c:if test="${selectStore[0].class4 == item.class4Seq}">selected="selected"</c:if>>${item.class3Nm}</option>
										</c:if>
										</c:forEach>
									</select>
				        		</td> 
					       	...
					       	<tr class="classAll" style="display: none;">
				        		...
				        		<td style="text-align: left;">
				        			<select id="class5" name="class5" class="zemos_select1" style="width:100%;" disabled="disabled">
										...
										<c:if test="${item.useYn == 'Y'}">
											<option value="${item.class3Seq}" <c:if test="${selectStore[0].class5 == item.class5Seq}">selected="selected"</c:if>>${item.class3Nm}</option>
										</c:if>
										</c:forEach>
									</select>
				        		</td> 
			        	...
			        	<tr> 
			        		...
			        		<td style="text-align: left;">
			        			<input type="checkbox" id="storeUseYn" name="storeUseYn" <c:if test="${selectStore[0].useYn == 'Y'}">checked="checked"</c:if>>
			...
			<div class="zemos_label_search1">
				<span class="fl mb5" style="width:100%; height: 29px; text-align: left;">
					<a href="javascript:void(0);" onclick="javascript:fnStoreUpdate(); return false;" class="zemos_form_span_btn_blue">
