...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			var trCntLength = $("#class5Table > tbody tr").length;
			var trSeq = 0;
			...
			//분류3명 keyup이벤트
			//$('input[name=class5Nm]').on('keyup keypress',function(){ 
			$('input[name=class5Nm]').on('keyup',function(){	
				var index = $(this).parent().parent().index();
				var rowIndex = index + 1;
				var class5Nm = $("#class5Nm"+rowIndex).val();
				var hclass5Nm = $("#hclass5Nm"+rowIndex).val();
		...
		function fn_reload(type, value1, value2) {
			...
			if ( class5Nm != '' || class5Nm != null ) {
				url += "&class5Nm=" + class5Nm;	
			}
			if ( useYn != '' || useYn != null ) {
		...
		//분류2 추가
		function fnClass5Add() {
			var innerHtml = "";
			var trCnt = $("#class5Table > tbody tr").length + 1;
			
			innerHtml += "<tr id=\"class5TableTr"+trCnt+"\">";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<input id=\"class5Nm"+trCnt+"\" name=\"class5Nm\" type=\"text\" class=\"zemos_input1\" style=\"float: left; width:100%;\" placeholder=\"${class5UseName[0].class5UseNm}명\"  maxlength=\"99\">";
	    	innerHtml += "	</td>";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<input id=\"useYn"+trCnt+"\" name=\"useYn\" type=\"checkbox\" class=\"checkbox-style\" style=\"display:block;\" checked=\"checked\">"; 
	    	innerHtml += "	</td>";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<a href=\"javascript:void(0);\" onclick=\"javascript:fnClass5Save('"+trCnt+"'); return false;\" class=\"zemos_form_span_btn_blue\">";
	    	...
	    	$("input[type=checkbox]").css("display", "block");
	        $("#class5Table > tbody:last").append(innerHtml);
	        $("#class5Nm"+trCnt).focus();
		}
		...
		//분류3저장
		function fnClass5Save(trCnt) {
			var useYn = "";
			var saveGbn = "";
			
			var class5Nm = $("#class5Nm"+trCnt).val();
			var class5Seq = $("#class5Seq"+trCnt).val();
			
			//class5Nm = class5Nm.replaceAll("","");
			
			
			if ( class5Nm == '' || class5Nm == null ) {
				alert("${class5UseName[0].class5UseNm}명을 입력해 주세요.");
				$("#class5Nm"+trCnt).focus();
				return;
			}
			
			if ( Number(byteCheck($("#class5Nm"+trCnt))) > 100 ) {
				alert("${class5UseName[0].class5UseNm}명의 글자수를 초과하였습니다.\n${class5UseName[0].class5UseNm}명은 한글 50글자, 영문 100글자까지 입니다.");
				$("#class5Nm"+trCnt).focus();
				return;
			}
			...
			params.useYn     = useYn;
			params.class5Seq = class5Seq;
			params.saveGbn   = saveGbn;
			params.zemosIdx  = zemosIdx;
			params.wrkplcIdx = wrkplcIdx;
			
			$.ajax({
				url 		: gvContextPath + "/zemos3/family/sales2/class5Save.do",
				type		: 'POST',
			    dataType 	: 'json',
			    data		: params
				...
				if ( response.result === true ) {
					alert("${class5UseName[0].class5UseNm}이 저장되었습니다.");
					fn_reload();
				} else {
					if ( response.resultError == '1' ) {
						alert("중복되는 ${class5UseName[0].class5UseNm}이 있습니다.");
					}
					fn_reload();
				}
		...
		function fnRowCheck(trCnt) {
			...
			if ( $("#class5Nm"+trCnt).val() == $("#hclass5Nm"+trCnt).val() && useYn == $("#huseYn"+trCnt).val()) {
		...
		function fnCheckBox(trCnt) {
			if ( $("#useYn"+trCnt).is(":checked") == true ) {
				$("#class5Nm"+trCnt).attr("disabled", false); 
			} else {
				$("#class5Nm"+trCnt).attr("disabled", true); 
			}
		}
...
<body>
    ...
    <div class="zemos_title1">
		<span class="zemos_title1_middle" onclick="javascript:fn_location_href();return false;">${class5UseName[0].class5UseNm} 등록</span>
    ...
    <div class="zemos_label_search1">
    	<span class="fl mb5" style="width:100%; padding-right:1%;">
    		...
    		<input id="class5Nm" name="class5Nm" type="text" class="zemos_input1" style="float:left; width:65%; margin-left: 4px;" value="${params.class5Nm[0]}" placeholder="검색어(${class5UseName[0].class5UseNm}명) 입력" onKeyPress="javascript:if(event.keyCode == 13) fn_reload('select');" onfocus="$(this).select();" maxlength="99">
    	</span>
    ...
    <div class="zemos_label_search2">
        <span class="fl" style="padding:10px 0px;">
			조회<span class="zemos_label_data_grey2">${fn:length(class5List)}</span>건
        </span>
        <span class="fr mb5" style="margin-top:5px; width:21%;">
			<a href="javascript:void(0);" onclick="javascript:fnClass5Add(); return false;" class="zemos_form_span_btn_red">
				<span style="font-size:13px;">추 가</span>
			</a>
		</span>
    </div>
    <div style="overflow-x:scroll; z-index:9999; -webkit-overflow-scrolling: touch;">
    	<table class="zemos_table1" id="class5Table" style="margin-top: 0px;">
    		<colgroup>
		        <col width="67%" />
		        <col width="13%" />
		        <col width="20%" />
	        ...
	        <thead>
	        	<tr>
	        		<th style="padding: 10px;">${class5UseName[0].class5UseNm}명</th>
	        ...
	        <tbody>
	        	<c:choose>
		        	<c:when test="${fn:length(class5List) > 0}">
		        		<c:forEach items="${class5List}" var="item" varStatus="status">
	        	<tr id="class5TableTr${status.count}">
	        		<td style="text-align: left;">
	        			<input id="class5Seq${status.count}" name="class5Seq" type="hidden" value="${item.class5Seq}">
	        			<input id="class5Nm${status.count}" name="class5Nm" value="${item.class5Nm}" type="text" class="zemos_input1" style="float: left; width:100%;" placeholder="${class5UseName[0].class5UseNm}명" maxlength="99">
	        			<span style="display:none;">${item.class5Nm}</span>
	        		</td>
	        		...
	        		<td style="text-align: center;">
	        			<a href="javascript:void(0);" onclick="javascript:fnClass5Save('${status.count}'); return false;" class="zemos_form_span_btn_blue" id="btnA${status.count}">
						...
						<input type="hidden" id="hclass5Nm${status.count}" name="hclass5Nm" value="${item.class5Nm}"/>
