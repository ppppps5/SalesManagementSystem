...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			var trCntLength = $("#class3Table > tbody tr").length;
			var trSeq = 0;
			...
			//분류3명 keyup이벤트
			//$('input[name=class3Nm]').on('keyup keypress',function(){ 
			$('input[name=class3Nm]').on('keyup',function(){	
				var index = $(this).parent().parent().index();
				var rowIndex = index + 1;
				var class3Nm = $("#class3Nm"+rowIndex).val();
				var hclass3Nm = $("#hclass3Nm"+rowIndex).val();
		...
		function fn_reload(type, value1, value2) {
			...
			if ( class3Nm != '' || class3Nm != null ) {
				url += "&class3Nm=" + class3Nm;	
			}
			if ( useYn != '' || useYn != null ) {
		...
		//분류2 추가
		function fnClass3Add() {
			var innerHtml = "";
			var trCnt = $("#class3Table > tbody tr").length + 1;
			
			innerHtml += "<tr id=\"class3TableTr"+trCnt+"\">";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<input id=\"class3Nm"+trCnt+"\" name=\"class3Nm\" type=\"text\" class=\"zemos_input1\" style=\"float: left; width:100%;\" placeholder=\"${class3UseName[0].class3UseNm}명\"  maxlength=\"99\">";
	    	innerHtml += "	</td>";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<input id=\"useYn"+trCnt+"\" name=\"useYn\" type=\"checkbox\" class=\"checkbox-style\" style=\"display:block;\" checked=\"checked\">"; 
	    	innerHtml += "	</td>";
	    	innerHtml += "	<td style=\"text-align: left;\">";
	    	innerHtml += "		<a href=\"javascript:void(0);\" onclick=\"javascript:fnClass3Save('"+trCnt+"'); return false;\" class=\"zemos_form_span_btn_blue\">";
	    	...
	    	$("input[type=checkbox]").css("display", "block");
	        $("#class3Table > tbody:last").append(innerHtml);
	        $("#class3Nm"+trCnt).focus();
		}
		...
		//분류3저장
		function fnClass3Save(trCnt) {
			var useYn = "";
			var saveGbn = "";
			
			var class3Nm = $("#class3Nm"+trCnt).val();
			var class3Seq = $("#class3Seq"+trCnt).val();
			
			//class3Nm = class3Nm.replaceAll("","");
			
			
			if ( class3Nm == '' || class3Nm == null ) {
				alert("${class3UseName[0].class3UseNm}명을 입력해 주세요.");
				$("#class3Nm"+trCnt).focus();
				return;
			}
			
			if ( Number(byteCheck($("#class3Nm"+trCnt))) > 100 ) {
				alert("${class3UseName[0].class3UseNm}명의 글자수를 초과하였습니다.\n${class3UseName[0].class3UseNm}명은 한글 50글자, 영문 100글자까지 입니다.");
				$("#class3Nm"+trCnt).focus();
				return;
			}
			...
			params.useYn     = useYn;
			params.class3Seq = class3Seq;
			params.saveGbn   = saveGbn;
			params.zemosIdx  = zemosIdx;
			params.wrkplcIdx = wrkplcIdx;
			
			$.ajax({
				url 		: gvContextPath + "/zemos3/family/sales2/class3Save.do",
				type		: 'POST',
			    dataType 	: 'json',
			    data		: params
				...
				if ( response.result === true ) {
					alert("${class3UseName[0].class3UseNm}이 저장되었습니다.");
					fn_reload();
				} else {
					if ( response.resultError == '1' ) {
						alert("중복되는 ${class3UseName[0].class3UseNm}이 있습니다.");
					}
					fn_reload();
				}
		...
		function fnRowCheck(trCnt) {
			...
			if ( $("#class3Nm"+trCnt).val() == $("#hclass3Nm"+trCnt).val() && useYn == $("#huseYn"+trCnt).val()) {
		...
		function fnCheckBox(trCnt) {
			if ( $("#useYn"+trCnt).is(":checked") == true ) {
				$("#class3Nm"+trCnt).attr("disabled", false); 
			} else {
				$("#class3Nm"+trCnt).attr("disabled", true); 
			}
		}
...
<body>
    ...
    <div class="zemos_title1">
		<span class="zemos_title1_middle" onclick="javascript:fn_location_href();return false;">${class3UseName[0].class3UseNm} 등록</span>
    ...
    <div class="zemos_label_search1">
    	<span class="fl mb5" style="width:100%; padding-right:1%;">
    		...
    		<input id="class3Nm" name="class3Nm" type="text" class="zemos_input1" style="float:left; width:65%; margin-left: 4px;" value="${params.class3Nm[0]}" placeholder="검색어(${class3UseName[0].class3UseNm}명) 입력" onKeyPress="javascript:if(event.keyCode == 13) fn_reload('select');" onfocus="$(this).select();" maxlength="99">
    	</span>
    ...
    <div class="zemos_label_search2">
        <span class="fl" style="padding:10px 0px;">
			조회<span class="zemos_label_data_grey2">${fn:length(class3List)}</span>건
        </span>
        <span class="fr mb5" style="margin-top:5px; width:21%;">
			<a href="javascript:void(0);" onclick="javascript:fnClass3Add(); return false;" class="zemos_form_span_btn_red">
				<span style="font-size:13px;">추 가</span>
			</a>
		</span>
    </div>
    <div style="overflow-x:scroll; z-index:9999; -webkit-overflow-scrolling: touch;">
    	<table class="zemos_table1" id="class3Table" style="margin-top: 0px;">
    		<colgroup>
		        <col width="67%" />
		        <col width="13%" />
		        <col width="20%" />
	        ...
	        <thead>
	        	<tr>
	        		<th style="padding: 10px;">${class3UseName[0].class3UseNm}명</th>
	        ...
	        <tbody>
	        	<c:choose>
		        	<c:when test="${fn:length(class3List) > 0}">
		        		<c:forEach items="${class3List}" var="item" varStatus="status">
	        	<tr id="class3TableTr${status.count}">
	        		<td style="text-align: left;">
	        			<input id="class3Seq${status.count}" name="class3Seq" type="hidden" value="${item.class3Seq}">
	        			<input id="class3Nm${status.count}" name="class3Nm" value="${item.class3Nm}" type="text" class="zemos_input1" style="float: left; width:100%;" placeholder="${class3UseName[0].class3UseNm}명" maxlength="99">
	        			<span style="display:none;">${item.class3Nm}</span>
	        		</td>
	        		...
	        		<td style="text-align: center;">
	        			<a href="javascript:void(0);" onclick="javascript:fnClass3Save('${status.count}'); return false;" class="zemos_form_span_btn_blue" id="btnA${status.count}">
						...
						<input type="hidden" id="hclass3Nm${status.count}" name="hclass3Nm" value="${item.class3Nm}"/>
