...
/**
 * Button 을 통해 APP 이 종료될 수 있도록 세팅
 * @returns
 * @type
 */
function fn_app_exitApp() {
	var str = "앱을 종료하시겠습니까?";
	var fnc = function (e) {
		if(e != null && e == true) {
			navigator.app.exitApp();
	...
	navigator.notification.confirm(str,fnc,"종료",['예','아니오']);
}

/**
 * Button 을 통해 APP 이 화면캡처 할 수 있도록 세팅
 * @returns
 * @type
 */
function fn_screenshot_save() {
	var str = "화면을 저장하시겠습니까?";
	var fnc = function (e) {
		if(e != null && e == true) {
	...
	navigator.notification.confirm(str,fnc,"스크린샷",['예','아니오']);
}

/**
 * Button 을 통해 APP 이 로그아웃 할 수 있도록 세팅
 * @returns
 * @type
 */
function fn_hyb_logout(url) {
	var str = "로그아웃 하시겠습니까?";
	var fnc = function (e) {
		if(e != null && e == true) {
			location.href = url;
	...
	navigator.notification.confirm(str,fnc,"로그아웃",['예','아니오']);
}
 ...
 * @type
 */
function fn_app_loadUrl(url) {
	navigator.app.loadUrl(url, {wait:2000, loadingDialog:"Wait,Loading App", loadUrlTimeoutValue: 60000});
}
