	...
	$.widget( "mobile.datebox", $.mobile.widget, {
		...
		_create: function() {
			...
			if ( o.useButton === true && o.useInline === false && o.useNewStyle === false ) {
				w.d.open = $('<a href="#" class="ui-input-clear" title="'+this.__('tooltip')+'">'+this.__('tooltip')+'</a>')
					...
					}).appendTo(w.d.wrap).buttonMarkup({icon: 'grid', iconpos: 'notext', corners:false, shadow:false})
		...
		_build: {
			'default': function () {
				...
				this.d.intHTML = $("<div class='ui-body-pop'><h2 style='text-align:center'>There is no mode by that name loaded / mode not given</h2></div>");
			}
		},
			...
			if ( o.enablePopup === true ) {
				...
				if ( o.useHeader === true ) {
					...
					$("<a class='ui-btn-left' href='#'>Close</a>").appendTo(w.d.headHTML)
						.buttonMarkup({ theme  : o.themeHeader, icon   : 'delete', iconpos: 'notext', corners: false, shadow : false })
